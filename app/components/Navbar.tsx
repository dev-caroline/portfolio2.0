...
const Navbar = ({ toggleSidenav }: { toggleSidenav: () => void }) => {
  const { initials, isVerified } = useAuth()
  ...
  return (
    <div className='flex justify-between items-center p-5 bg-black/20 backdrop-blur-sm border-b border-red-500/20 sticky top-0 z-20'>
      ...
      {/* Profile: initials avatar once verified, person icon otherwise */}
      {isVerified && initials ? (
        <div className='h-9 w-9 rounded-full bg-black border border-red-500/50 flex items-center justify-center text-white font-semibold text-sm shadow-lg shadow-red-500/20 select-none'>
          {initials}
        </div>
      ) : (
