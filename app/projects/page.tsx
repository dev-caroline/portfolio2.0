          ...
          <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
            {PROJECTS_LIST.map((project) => (
              <div key={project.id} className='group relative flex flex-col h-fit'>
                ...
                <div className='w-full h-40 bg-black/60 border-2 border-red-500/20 rounded-lg m-3 flex items-center justify-center overflow-hidden relative'>
                    {project.image ? <Image src={project.image} alt={project.name} fill sizes='(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw' className='object-cover' /> : null}
                  </div>
